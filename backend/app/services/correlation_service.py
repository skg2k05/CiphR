...
async def run_correlation(sample_id: str, db: AsyncSession):
    ...
    for match in all_analyses:
        ...
        # Exclude generic reflection API (Method.invoke) to prevent false correlations across framework/demo code
        filtered_match_apis = set(a for a in match_apis if a != "java.lang.reflect.Method.invoke")
        filtered_curr_apis = set(a for a in dex_apis if a != "java.lang.reflect.Method.invoke")

        shared_ips = dex_ips.intersection(match_ips)
        shared_domains = dex_domains.intersection(match_domains)
        shared_apis = filtered_curr_apis.intersection(filtered_match_apis)
